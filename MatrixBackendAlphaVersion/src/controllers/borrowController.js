...
exports.borrowBook = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    const notification = await Notification.create({
      user: student,
      message: `"${book.title}" has been borrowed. Due date: ${new Date(dueDate).toLocaleDateString()}`,
      type: 'book',
      meta: { bookId: bookId, dueDate }
    });

    // 🚀 WebSocket: Emit book borrowed event
    emitBookBorrowed({
      student: { _id: student._id, name: student.name, rollNumber: student.rollNumber },
      book: { id: bookId, title: book.title },
      borrowDate: borrowRecord[0].borrowDate,
      dueDate: borrowRecord[0].dueDate,
      issuedBy: req.user.name
    });

    // 🚀 WebSocket: Send notification to student
    emitNotification({
      userId: student._id,
    ...
    // 🚀 WebSocket: Update analytics in real-time
    emitAnalyticsUpdate('borrow', {
      bookId,
...
exports.returnBook = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    const notification = await Notification.create({
      user: record.student,
      message: fine > 0
        ? `"${book.title}" has been returned. Fine: ₹${fine}`
        : `"${book.title}" has been returned successfully.`,
      type: fine > 0 ? 'warning' : 'success',
      meta: { bookId: record.book, fine }
    });

    // 🚀 WebSocket: Emit book returned event
    emitBookReturned({
      student: {
      ...
      book: { id: record.book, title: book.title },
...
exports.getAdminAnalytics = asyncHandler(async (req, res) => {
  ...
  // Get daily traffic for past 6 days (Mon-Sat)
  ...
  // Initialize 6-day array with zeros for Mon-Sat
  const dailyTrafficData = Array(6).fill(0);
  const days = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];
  const trafficMap = {};

  dailyTraffic.forEach(item => {
    trafficMap[item._id] = item.count;
  });

  // Fill in the data for the past 6 days
  for (let i = 0; i < 6; i++) {
    const date = new Date(sixDaysAgo);
    date.setDate(date.getDate() + i);
    const dateStr = date.toISOString().split('T')[0];
    dailyTrafficData[i] = trafficMap[dateStr] || 0;
  }
  ...
  const borrowCountsMap = {};
  borrowCounts.forEach(item => {
    borrowCountsMap[item._id.toString()] = item.totalBorrows;
  });

  // Create leaderboard with hours spent and books borrowed
  ...
  for (const [rollNumber, timeData] of Object.entries(studentTimeSpent)) {
    ...
    if (student) {
      ...
      const totalBorrows = borrowCountsMap[student._id.toString()] || 0;
